  ...
  const decodedToken = accessToken ? jwtDecode(accessToken) : null;
    ...
    useEffect(() => {
            const fetchData = async() => {
                try {
                    const res = await getResource(`/sequences/${id}`);
    ...
    const updateData = (values) => {
      console.log(values)
        ...
        postFile(`/sequences/${id}`, values).then((res) => {
            onServerSuccess("Mise à jour effectuée avec succès.")
            formik.resetForm();
            setDatas({content: ""})
            setTimeout(() => navigate(`/admin/sequences`), 50)
    ...
    const saveData = (data) => {
        ...
        postFile("/sequences", data).then((res) => {
    ...
    return ( 
        <Body isOpen={isOpen} setIsOpen={setIsOpen}>
            <FormBody title={!id ? "Creation de Chapitre de module" : "Mise a jour "}>
            <form className="flex flex-col w-full items-center" onSubmit={formik.handleSubmit}><br />
                      <div className="w-full mb-2 flex flex-col">
                            <label className=" font-semibold mb-2 text-gray-800">Titre du chapitre ou sequence  <span className="text-red-500">*</span></label>
